...
class EventSummary:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "EventSummary":
        ...
        home_player = Player(
            ...
            image_id=home_data.get("image_id") if home_data.get("image_id", 0) != 0 else None
        )
        ...
        away_player = Player(
            ...
            image_id=away_data.get("image_id") if away_data.get("image_id", 0) != 0 else None
        )
    ...
    def is_scheduled(self) -> bool:
        ...
        return self.time_status in ["0", "1"]  # "0" = upcoming, "1" = scheduled/about to start
...
class Event:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Event":
        ...
        home_player = Player(
            ...
            image_id=data["home"].get("image_id") if data["home"].get("image_id", 0) != 0 else None
        )
        ...
        away_player = Player(
            ...
            image_id=data["away"].get("image_id") if data["away"].get("image_id", 0) != 0 else None
        )
    ...
    def winner(self) -> Optional[Player]:
        ...
        if not self.is_finished:
            return None
        return self.home_player if self.home_sets_won > self.away_sets_won else self.away_player
...
class PlayerMatchHistory:
    ...
    def recent_form(self) -> List[bool]:
        ...
        sorted_matches = sorted(self.matches, key=lambda m: m.event_datetime or datetime.min)
        return [match.is_winner(self.player_name) for match in sorted_matches[-10:]]  # Last 10 matches
    
    @classmethod
    def from_matches(cls, 
                    player_name: str, 
                    matches: List[EventSummary],
                    h2h_records: Dict[str, List[EventSummary]],
                    date_range_days: int) -> "PlayerMatchHistory":
        ...
        tournaments = list(set(match.league_name for match in matches if match.league_name))
        opponents = list(set(
            match.away_player.name if match.home_player.name == player_name 
...
class TournamentData:
    ...
    def from_matches(cls,
                    ...
                    tournament_name: str, 
                    matches: List[EventSummary],
                    has_odds_data: bool = False) -> "TournamentData":
        """Create from list of matches with calculated metadata"""
