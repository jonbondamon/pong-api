...
class TableTennisAPI:
    ...
    def _make_request(self, method: str, endpoint: str, version: str = "v3", **kwargs) -> Dict[str, Any]:
        ...
        base_url = self.base_url.rstrip('/').rsplit('/', 1)[0]  # Remove version if present
        ...
        if 'sport_id' not in kwargs['params'] and any(endpoint.startswith(ep) for ep in endpoints_needing_sport_id):
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise RateLimitError(f"Rate limit exceeded. Resets at: {self.rate_limit_reset}")
