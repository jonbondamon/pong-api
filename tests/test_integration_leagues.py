...
from tabletennis_api.models import League, PaginationInfo, APIResponse
...
class TestLeagueIntegration:
    ...
    def test_real_league_list_basic(self, real_api_client):
        ...
        print(f"⏱️  Rate limit: {real_api_client.rate_limit_remaining}/{real_api_client.rate_limit}")
    ...
    def test_real_league_pagination(self, real_api_client):
        ...
        if page1.pagination.has_next_page:
            ...
            assert page1_ids.isdisjoint(page2_ids), "Pages should have different leagues"
...
class TestLeagueIntegrationAdvanced:
    ...
    def test_real_list_all_leagues_partial(self, real_api_client):
        ...
        print(f"✅ Successfully tested {len(all_leagues)} leagues across {max_pages_to_test} pages")
        ...
        for league in leagues_with_tables[:2]:  # Test first 2 to limit API calls
            ...
            try:
                ...
                if isinstance(table_data, list):
                    ...
                    print(f"   ✅ Table has nested structure with keys: {list(table_data.keys())}")
    ...
    def test_real_league_rankings_functionality(self, real_api_client):
        ...
        for page in range(1, max_pages_to_search + 1):
            ...
            for league in response.results:
                if league.supports_rankings:
                    ...
                    if len(leagues_with_rankings) >= 3:  # We only need a few for testing
                        break
        ...
        if not leagues_with_rankings:
            ...
            print("ℹ️  No leagues with rankings support found (expected based on documentation)")
            
            # Test with a league that doesn't support rankings to verify error handling
            response = real_api_client.leagues.list(page=1)
            if response.results:
                test_league = response.results[0]
                print(f"🧪 Testing rankings on non-supporting league: {test_league.name}")
                ...
                try:
                    ...
                    print(f"   ✅ Returned {len(rankings_data)} ranking entries (expected to be 0 or minimal)")
                except TableTennisAPIError as e:
...
if __name__ == "__main__":
    ...
    pytest.main([
        "tests/test_integration_leagues.py",
        "-v",
        "-m", "integration",
        "--tb=short"
    ])