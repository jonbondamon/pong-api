...
class TestBulkEventsManagerIntegration:
    ...
    def test_real_get_player_history_basic(self, real_api_client):
        ...
        try:
            ...
            # Verify return type
            assert isinstance(history, PlayerMatchHistory), "Should return PlayerMatchHistory object"
            
            # Verify basic structure
            assert history.player_name == player_name, f"Player name should be '{player_name}'"
            ...
            assert history.total_matches == len(history.matches), "Total matches should match list length"
            assert history.win_count + history.loss_count == history.total_matches, "Wins + losses should equal total"
            assert history.date_range_days == 7, "Date range should match input"
            
            if history.total_matches > 0:
                assert 0.0 <= history.win_rate <= 1.0, "Win rate should be between 0 and 1"
                ...
                assert all(isinstance(result, bool) for result in recent_form), "Recent form should contain booleans"
                print(f"      Recent form: {['W' if win else 'L' for win in recent_form[-5:]]}")  # Last 5
            else:
    ...
    def test_real_get_player_history_with_h2h(self, real_api_client):
        ...
        try:
            ...
            if history.h2h_records:
                ...
                for opponent, h2h_matches in list(history.h2h_records.items())[:2]:  # Show first 2
                    ...
                    if h2h_matches:
                        for match in h2h_matches[:1]:  # Check first match
                            ...
                            assert hasattr(match, 'home_player'), "H2H match should have home player"
                            assert hasattr(match, 'away_player'), "H2H match should have away player"
            else:
    ...
    def test_real_get_player_history_extended_period(self, real_api_client):
        ...
        try:
            ...
            if history.total_matches > 0:
                ...
                actual_wins = sum(1 for match in history.matches if match.is_winner(player_name))
                actual_losses = history.total_matches - actual_wins
                
                assert history.win_count == actual_wins, f"Win count mismatch: {history.win_count} != {actual_wins}"
                assert history.loss_count == actual_losses, f"Loss count mismatch: {history.loss_count} != {actual_losses}"
                ...
                for match in history.matches:
                    ...
                    opponent = (match.away_player.name if match.home_player.name == player_name 
                               else match.home_player.name)
                    unique_opponents.add(opponent)
                
                assert len(history.tournaments) == len(unique_tournaments), "Tournament extraction mismatch"
                assert len(history.opponents) == len(unique_opponents), "Opponent extraction mismatch"
    ...
    def test_real_get_player_history_error_handling(self, real_api_client):
        ...
        try:
            ...
            assert isinstance(history, PlayerMatchHistory), "Should return PlayerMatchHistory even for unknown player"
            assert history.total_matches == 0, "Should have no matches for unknown player"
    ...
    def test_real_get_player_history_performance(self, real_api_client):
        ...
        try:
            ...
            if initial_remaining is not None and final_remaining is not None:
                ...
                if history.total_matches > 0:
                    print(f"   🎯 Efficiency: {history.total_matches / max(api_calls_used, 1):.2f} matches per API call")
                
                # Performance assertions
                assert api_calls_used < 20, "Should use less than 20 API calls for this test"
            else:
    ...
    def test_real_helper_methods(self, real_api_client):
        ...
        try:
            ...
            for match in recent_matches:
                assert (match.home_player.name == "Serhii Pitsyk" or 
                       match.away_player.name == "Serhii Pitsyk"), "Match should contain target player"
            
            if recent_matches:
                # Test _get_h2h_matches with real data
                print(f"   🤝 Testing _get_h2h_matches helper...")
                first_match = recent_matches[0]
                opponent = (first_match.away_player.name if first_match.home_player.name == "Serhii Pitsyk"
                           else first_match.home_player.name)
                
                h2h_matches = real_api_client.events._get_h2h_matches("Serhii Pitsyk", opponent, 30)
                ...
                for match in h2h_matches:
                    ...
                    assert "Serhii Pitsyk" in players, "H2H match should include Serhii Pitsyk"
...
class TestTournamentCompleteIntegration:
    ...
    def test_real_get_tournament_complete_basic(self, real_api_client):
        ...
        try:
            ...
            print(f"   🏆 Testing with tournament: {tournament_name} (ID: {tournament_id})")
            ...
            # Verify return type
            assert isinstance(tournament, TournamentData), "Should return TournamentData object"
            
            # Verify basic structure
            assert tournament.tournament_id == str(tournament_id), f"Tournament ID should be '{tournament_id}'"
            assert tournament.tournament_name == tournament_name, f"Tournament name should be '{tournament_name}'"
            ...
            assert tournament.total_matches == len(tournament.matches), "Total matches should match list length"
            assert tournament.completed_matches + tournament.live_matches + tournament.upcoming_matches == tournament.total_matches
            
            if tournament.total_matches > 0:
                assert 0.0 <= tournament.completion_rate <= 1.0, "Completion rate should be between 0 and 1"
                
                print(f"   🏓 Sample matches:")
                for match in tournament.matches[:3]:
                    status = "Completed" if match.is_finished else ("Live" if match.is_live else "Upcoming")
                    print(f"      {match.home_player.name} vs {match.away_player.name} [{status}]")
                
                # Verify date range
                if tournament.date_range[0] and tournament.date_range[1]:
                    assert tournament.date_range[0] <= tournament.date_range[1], "Date range should be properly ordered"
                    print(f"      Date range: {tournament.date_range[0]} to {tournament.date_range[1]}")
            else:
    ...
    def test_real_get_tournament_complete_with_odds(self, real_api_client):
        ...
        try:
            ...
            assert isinstance(tournament.has_odds_data, bool), "has_odds_data should be boolean"
    ...
    def test_real_get_tournament_complete_all_match_types(self, real_api_client):
        ...
        try:
            ...
            assert len(match_ids) == len(set(match_ids)), "Should have no duplicate matches"
            ...
            assert tournament.completed_matches == actual_completed, "Completed count mismatch"
            assert tournament.live_matches == actual_live, "Live count mismatch"
            assert tournament.upcoming_matches == actual_upcoming, "Upcoming count mismatch"
    ...
    def test_real_get_tournament_complete_error_handling(self, real_api_client):
        ...
        try:
            ...
            assert isinstance(tournament, TournamentData), "Should return TournamentData even for unknown tournament"
            assert tournament.total_matches == 0, "Should have no matches for unknown tournament"
            assert tournament.tournament_id == "99999999", "Should preserve tournament ID"
            assert tournament.tournament_name == "Tournament 99999999", "Should have fallback name"
    ...
    def test_real_helper_method_get_tournament_matches(self, real_api_client):
        ...
        try:
            ...
            for match in ended_matches:
                assert match.league_id == str(tournament_id), f"Match should be from tournament {tournament_id}"
...
class TestEventsBulkIntegration:
    ...
    def test_real_get_events_bulk_basic(self, real_api_client):
        ...
        try:
            ...
            assert len(events) == len(event_ids), f"Should retrieve all {len(event_ids)} events"
        ...
        try:
            ...
            try:
                ...
                if upcoming_events.results:
                    ...
                    print(f"      ✓ Found upcoming event: {upcoming_events.results[0].id}")
            except Exception:
            ...
            assert len(events) == len(collected_ids), f"Should retrieve all {len(collected_ids)} events"
            
            for event_id in collected_ids:
                assert event_id in events, f"Event {event_id} should be in results"
                event = events[event_id]
                
                status = "Ended" if event.is_finished else ("Live" if event.is_live else "Upcoming")
                print(f"      ✓ {event.home_player.name} vs {event.away_player.name} [{status}]")
    ...
    def test_real_get_events_bulk_performance(self, real_api_client):
        ...
        try:
            ...
            if initial_remaining is not None and final_remaining is not None:
                ...
                assert api_calls_used < len(event_ids), "Bulk method should be more efficient"
