...
class TestOddsManagerIntegration:
        ...
        try:
            ...
            if odds_summary:
                ...
                for bookmaker, data in odds_summary.items():
                    ...
                    if "odds" in data:
                        odds_data = data["odds"]
                        print(f"      📈 Has odds data with {len(odds_data)} categories")
                        
                        # Check for different odds categories
                        for category, category_odds in odds_data.items():
                            if category_odds:
                                print(f"         {category}: {len(category_odds)} markets")
                                
                                # Show sample odds for each category
                                for market_id, odds in list(category_odds.items())[:2]:
                                    odds_info = []
                                    if "home_od" in odds and "away_od" in odds:
                                        odds_info.append(f"1x2: {odds['home_od']}/{odds['away_od']}")
                                    if "handicap" in odds:
                                        odds_info.append(f"Handicap: {odds['handicap']}")
                                    if "over_od" in odds and "under_od" in odds:
                                        odds_info.append(f"O/U: {odds['over_od']}/{odds['under_od']}")
                                    
                                    if odds_info:
                                        print(f"            {market_id}: {', '.join(odds_info)}")
                            else:
                ...
                found_bookmakers = [bm for bm in expected_bookmakers if bm in odds_summary]
                print(f"   🎯 Found {len(found_bookmakers)}/{len(expected_bookmakers)} expected bookmakers")
    ...
    def test_real_odds_detailed_functionality(self, real_api_client):
        ...
        try:
            # Get detailed odds
            print(f"   📊 Getting detailed odds for event {event_id} from {bookmaker}...")
            ...
            if detailed_odds:
                    ...
                    for category, odds_history in odds_data.items():
                        if odds_history and isinstance(odds_history, list):
                            ...
                            if odds_history:
                                ...
                                print(f"         Latest: {latest_odds.get('ss', 'N/A')} score")
                                if "home_od" in latest_odds and "away_od" in latest_odds:
                                    print(f"         Latest odds: {latest_odds['home_od']}/{latest_odds['away_od']}")
                                
                                print(f"         Opening: {oldest_odds.get('ss', 'N/A')} score")
                                if "home_od" in oldest_odds and "away_od" in oldest_odds:
                                    print(f"         Opening odds: {oldest_odds['home_od']}/{oldest_odds['away_od']}")
                        else:
                ...
                expected_categories = ["92_1", "92_2", "92_3"]  # Match winner, Handicap, Over/Under
                found_categories = [cat for cat in expected_categories if cat in detailed_odds.get("odds", {})]
                print(f"   🎯 Found {len(found_categories)}/{len(expected_categories)} expected odds categories")
    ...
    def test_real_odds_with_events_integration(self, real_api_client):
        ...
        try:
            ...
            if ended_events.results:
                ...
                print(f"   🏓 Testing with event: {test_event.home_player.name} vs {test_event.away_player.name}")
                ...
                try:
                    ...
                    if odds_summary:
                        print(f"      ✅ Found odds data from {len(odds_summary)} bookmakers")
                        
                        # Try detailed odds
                        detailed_odds = real_api_client.odds.get_detailed(test_event.id)
                        if detailed_odds and "odds" in detailed_odds:
                            print(f"      ✅ Found detailed odds with {len(detailed_odds['odds'])} categories")
                        else:
    ...
    def test_real_odds_error_handling(self, real_api_client):
        ...
        try:
            ...
            if not odds_summary:
                print(f"      ✅ API correctly returned empty results for invalid event")
            else:
        ...
        try:
            ...
            if not detailed_odds:
                print(f"      ✅ Detailed odds correctly returned empty for invalid event")
            else:
                print(f"      ⚠️  Detailed odds returned data for invalid event (unexpected)")
                
        except TableTennisAPIError as e:
            print(f"      ✅ Detailed odds correctly raised error for invalid event: {e}")